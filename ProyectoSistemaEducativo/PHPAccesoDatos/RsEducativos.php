...
try {
    ...
    if (isset($_POST['agregar'])) {
        $sql = "INSERT INTO recursos_educativos (titulo, descripcion, enlace) VALUES (?, ?, ?)";
    ...
    if (isset($_GET['eliminar'])) {
        $sql = "DELETE FROM recursos_educativos WHERE id = ?";
        $stmtDelete = $pdo->prepare($sql);
        $stmtDelete->execute([$_GET['eliminar']]);
    }

    // --- CONSULTAR ---
    $sql = "SELECT id, titulo, descripcion, enlace FROM recursos_educativos";
    $stmt = $pdo->query($sql);
...
<title>Recursos Educativos | EduAmbiente</title>
...
<div class="container-fluid col-10 p-4">
  ...
  <h2 class="text-success">Recursos Educativos</h2>
  <p>
    En este apartado nuestra intención es compartir diferentes materiales para que usted se pueda educar y tomar conciencia.
  </p>

  <!-- Formulario para agregar -->
  <div class="card p-3 mb-4">
    <h4 class="text-success">Agregar nuevo recurso educativo</h4>
    <form method="POST" action="RsEducativos.php">
      <div class="mb-3">
  ...
  <!-- Lista de recursos -->
  <h3 class="text-success mb-4">Recursos disponibles</h3>
  <div class="row g-4">
      ...
      <div class="col-md-6">
        <div class="card h-100">
          <div class="card-body">
            ...
            <?php endif; ?>
            <a href="RsEducativos.php?eliminar=<?= $row['id'] ?>" class="btn btn-danger btn-sm mt-2">Eliminar</a>
          </div>
        </div>
      </div>
    <?php endwhile; ?>
